...
Use a focused branch and include: user-visible effect, security/manual-recovery impact, tests run, UE fixture/cleanup details, and paired documentation changes. Public PR CI is intentionally limited to GitHub-hosted checks. A maintainer runs protected UE CI only after the code reaches a trusted ref.
...
分支和 PR 保持单一主题，并写清：用户可见变化、安全/手工还原影响、已运行测试、UE Fixture 与清理方式、中英文文档变更。公开 PR CI 只做 GitHub-hosted 检查；代码进入可信 ref 后，维护者才运行受保护 UE CI。
