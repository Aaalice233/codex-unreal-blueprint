...
- 创建、复制、重命名、移动、删除、重设父类、Interface 变更、Redirector 修复、检查、校验、写入、验证和恢复。

每种写操作都必须共用 Operation Registry，并经过完整的预检到恢复链路。只有功能覆盖、没有安全链路，不算满足 v1。
...
产品不维护长期版本化 Plan 文件。Apply 接受当次普通请求，持久化状态仅保留 Job Journal 和恢复/审计记录。
