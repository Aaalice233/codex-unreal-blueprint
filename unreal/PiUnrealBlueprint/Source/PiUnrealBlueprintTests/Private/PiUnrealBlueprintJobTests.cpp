...
namespace
{
    ...
    {
        return FPaths::Combine(FPaths::ProjectSavedDir(), TEXT("PiUnrealBlueprintTests"),
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FPiRequestJournalIdempotencyTest,
    "PiUnrealBlueprint.Idempotency.CompletedResponseLossAndConflict", EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FPiRequestJournalIdempotencyTest::RunTest(const FString& Parameters)
{
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FPiRequestJournalConcurrentDuplicateTest,
    "PiUnrealBlueprint.Idempotency.ConcurrentDuplicate", EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FPiRequestJournalConcurrentDuplicateTest::RunTest(const FString& Parameters)
{
    const FString Directory = MakeJournalTestDirectory();
    FScopedDirectory DirectoryCleanup(Directory);
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FPiRequestJournalRestartRecoveryTest,
    "PiUnrealBlueprint.Idempotency.RestartMarksInflightInterrupted", EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FPiRequestJournalRestartRecoveryTest::RunTest(const FString& Parameters)
{
    const FString Directory = MakeJournalTestDirectory();
    FScopedDirectory DirectoryCleanup(Directory);
    FProtocolError Error;
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FPiRequestJournalCorruptionTest,
    "PiUnrealBlueprint.Idempotency.CorruptJournalFailsExplicitly", EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FPiRequestJournalCorruptionTest::RunTest(const FString& Parameters)
{
    const FString Directory = MakeJournalTestDirectory();
    FScopedDirectory DirectoryCleanup(Directory);
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FPiJobReadConcurrencyTest,
    "PiUnrealBlueprint.Jobs.ConcurrentReads", EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FPiJobReadConcurrencyTest::RunTest(const FString& Parameters)
{
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FPiJobSingleWriterTest,
    "PiUnrealBlueprint.Jobs.GlobalSingleWriter", EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FPiJobSingleWriterTest::RunTest(const FString& Parameters)
{
    FJobManager& Manager = FJobManager::Get();
    FThreadSafeCounter Active;
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FPiJobSafeCancellationTest,
    "PiUnrealBlueprint.Jobs.SafeCancellation", EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FPiJobSafeCancellationTest::RunTest(const FString& Parameters)
{
    FJobManager& Manager = FJobManager::Get();
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FPiJobCancellationTest,
    "PiUnrealBlueprint.Jobs.UnsafeCancellationPreservesFailure", EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FPiJobCancellationTest::RunTest(const FString& Parameters)
{
    FJobManager& Manager = FJobManager::Get();
    ...
    const FString JobId = Manager.Start(EJobAccess::Write,
        ...
        [&EnteredUnsafe, &ReleaseUnsafe](FJobExecutionContext& Context, TSharedPtr<FJsonObject>& Result, FProtocolError& Error)
        {
            ...
            Error = FProtocolError::Make(EErrorCode::VerificationFailed,
                TEXT("Saved assets could not be verified."), TEXT("FPiJobCancellationTest"));
            return false;
        });
    const double Deadline = FPlatformTime::Seconds() + 1.0;
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FPiJobQueueLimitTest,
    "PiUnrealBlueprint.Jobs.QueueLimit", EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FPiJobQueueLimitTest::RunTest(const FString& Parameters)
{
    FJobManager& Manager = FJobManager::Get();
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FPiWriteLeaseHeartbeatTest,
    "PiUnrealBlueprint.Jobs.WriteLeaseHeartbeatExpiry", EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FPiWriteLeaseHeartbeatTest::RunTest(const FString& Parameters)
{
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FPiTimedOutWriterRecoveryTest,
    "PiUnrealBlueprint.Jobs.TimedOutWriterRecovery", EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FPiTimedOutWriterRecoveryTest::RunTest(const FString& Parameters)
{
    FJobManager& Manager = FJobManager::Get();
