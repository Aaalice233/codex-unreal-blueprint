...
namespace CodexUnrealBlueprintTests
{
    ...
    {
        FString RequestedRunId = FPlatformMisc::GetEnvironmentVariable(TEXT("CODEX_UNREAL_AUTOMATION_RUN_ID"));
        if (RequestedRunId.IsEmpty()) RequestedRunId = TEXT("local");
        RunId = Sanitize(RequestedRunId + TEXT("_") + SuiteName + TEXT("_")
            + FGuid::NewGuid().ToString(EGuidFormats::Digits));
        Root = TEXT("/Game/CodexAutomation/") + RunId;
    }
    ...
    FScopedFixture::~FScopedFixture()
    {
        ...
        if (!Cleanup(&Error))
        {
            UE_LOG(LogTemp, Error, TEXT("CodexUnrealBlueprint fixture cleanup failed for '%s': %s"), *Root, *Error);
        }
    }
    ...
    {
        const FBlueprintOperationResult Result = CodexUnrealBlueprint::FBlueprintAssetOperations::Create(
            Package(Leaf), CodexUnrealBlueprint::EBlueprintAssetKind::Blueprint,
    ...
    {
        const FBlueprintOperationResult Result = CodexUnrealBlueprint::FBlueprintAssetOperations::Create(
            Package(Leaf), CodexUnrealBlueprint::EBlueprintAssetKind::UserDefinedStruct, nullptr);
    ...
    {
        const FBlueprintOperationResult Result = CodexUnrealBlueprint::FBlueprintAssetOperations::Create(
            Package(Leaf), CodexUnrealBlueprint::EBlueprintAssetKind::UserDefinedEnum, nullptr);
    ...
    FScopedDirectory::~FScopedDirectory()
    {
        ...
        if (!Cleanup(&Error))
        {
            UE_LOG(LogTemp, Error, TEXT("CodexUnrealBlueprint temporary directory cleanup failed: %s"), *Error);
        }
    }
