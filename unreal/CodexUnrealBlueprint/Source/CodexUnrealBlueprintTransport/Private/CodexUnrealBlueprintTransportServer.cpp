...
DEFINE_LOG_CATEGORY_STATIC(LogCodexUnrealBlueprintServer, Log, All);
...
namespace CodexUnrealBlueprint
{
    ...
    namespace
    {
        ...
        bool ApplyCurrentUserOnlyAcl(const FString& Path, const bool bDirectory, FString& OutError)
        {
            ...
            return true;
#else
            OutError = TEXT("CodexUnrealBlueprint Transport v1 supports Win64 only.");
            return false;
#endif
        }
    ...
    public:
        ...
        bool Start()
        {
            ...
            Thread = FRunnableThread::Create(this, TEXT("CodexUnrealBlueprintConnection"), 128 * 1024, TPri_Normal);
        ...
        void QueueJson(const FString& Json)
        {
            ...
            {
                UE_LOG(LogCodexUnrealBlueprintServer, Error, TEXT("Cannot encode JSON-RPC message: %s"), *Error);
                bStopping = true;
                return;
            }
            ...
            {
                UE_LOG(LogCodexUnrealBlueprintServer, Error, TEXT("Cannot queue the transport overflow error: %s"), *Error);
                bStopping = true;
                return;
            }
    ...
    public:
        bool Start(const FTransportServerConfig& Config, FProtocolError& OutError)
        {
            ...
            ListenSocket = SocketSubsystem->CreateSocket(NAME_Stream, TEXT("CodexUnrealBlueprint loopback listener"), BindAddress->GetProtocolType());
        ...
        bool WriteSessionDescriptor(const FTransportServerConfig& Config, FProtocolError& OutError)
        {
            ...
            if (SessionDirectory.IsEmpty())
            {
                ...
                SessionDirectory = FPaths::Combine(LocalAppData, TEXT("CodexUnrealBlueprint"), TEXT("sessions"));
            }
