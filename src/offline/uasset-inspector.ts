...
export async function inspectOfflineAsset(filePath: string, contentRoot?: string, searchTerms: string[] = []): Promise<JsonValue> {
  const arguments_ = ["-Path", resolve(filePath), "-Format", "json", "-Search", ...(searchTerms.length > 0 ? searchTerms : [""])];
  if (contentRoot !== undefined) arguments_.push("-ContentRoot", resolve(contentRoot));
...
export async function compareOfflineAssets(baseFilePath: string, targetFilePath: string, contentRoot?: string,
  searchTerms: string[] = []): Promise<JsonValue> {
  ...
  const base = await inspectOfflineAsset(baseFilePath, contentRoot, searchTerms);
  const target = await inspectOfflineAsset(targetFilePath, contentRoot, searchTerms);
